...
use crate::conversation_sandbox::{
    ...
    conversation_sandboxes, ensure_conversation_sandbox, selected_conversation_sandbox,
};
...
pub(crate) async fn ensure_shell_sandbox(
    ...
    // An explicit binding wins: it is the only thing that can name a sandbox
    // the conversation did not create, such as one restored from a snapshot
    // whose image no longer matches this conversation's configured spec.
    if let Some(sandbox_id) = selected_conversation_sandbox(conversation).await? {
