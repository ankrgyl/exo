...
Environment overrides:
  ...
  OPENAI_API_KEY, OPENROUTER_API_KEY, VENICE_API_KEY
EOF
}
...
configure_model_provider() {
  ...
  case "$provider" in
    ...
    openrouter)
      ...
      ;;
    venice)
      MODEL_PROVIDER_LABEL="Venice"
      MODEL_API_KEY_ENV="VENICE_API_KEY"
      MODEL_BASE_URL="$DEFAULT_VENICE_BASE_URL"
      DEFAULT_UPSTREAM_MODEL="$DEFAULT_VENICE_MODEL"
      ;;
    *)
      die "unsupported model provider: $provider" \
        "(expected openai, openrouter, or venice)"
      ;;
  esac
}
...
main() {
  ...
  if [[ -z "$UPSTREAM_MODEL" ]]; then
    if [[ "$MODEL_PROVIDER" == "openrouter" || "$MODEL_PROVIDER" == "venice" ]]; then
