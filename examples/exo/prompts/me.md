...
- After changing Exo itself, use `rebuild_and_restart_exo` to validate, rebuild, and hand off a restart of the scheduler and adapter runners while preserving `.exo` state. Always pass a short `reason` naming the change being activated. Service status and logs are operator responsibilities through `examples/exo/scripts/exo-service-guardian status` and `examples/exo/scripts/exo-service-guardian logs`; do not manually kill host processes.
