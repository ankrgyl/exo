...
export function registerIntrospectionTools(
  ...
  registerHostTool(registry, {
    ...
    description:
      'List this conversation\'s canonical event log, newest first. By default returns lifecycle and host events only: conversation_created, conversation_forked, session_started, session_ended, error, sandbox_created, sandbox_started, sandbox_stopped, sandbox_snapshotted, host_reboot (planned host restart with reason), adapter_runner_started (a start without a preceding host_reboot implies a crash or manual restart), and adapter_runner_draining (graceful shutdown began), exo.compaction.v1 (older history was replaced by a summary), exo.compaction.failed.v1, and exo.compaction.usage.v1 (what a compaction summarizer call cost). Pass explicit kinds to query other event types (e.g. tool_requested, tool_result, messages — these can be very large). Each messages event carries a usage annotation with token counts and cost_usd for that model turn, so this tool is also how to answer questions about spend: query kinds ["messages", "exo.compaction.usage.v1"] and sum cost_usd across both, because compaction summarizer calls are billable and are recorded on their own event rather than as messages. Use this to reconstruct restarts, crashes, session history, and per-conversation cost. Read-only.',
    parameters: {
      type: "object",
      additionalProperties: false,
      properties: {
